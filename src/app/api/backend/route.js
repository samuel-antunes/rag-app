...
async function searchEngineForSources(message, to) {
  ...
  sendPayload({ type: "Sources", content: normalizedData, to: to });
...
async function triggerLLMAndFollowup(inputString, to) {
  // 28. Call getGPTResults with inputString
  await getGPTResults(inputString, to);
  ...
  sendPayload({ type: "FollowUp", content: followUpResult, to: to });
  // 31. Return JSON response
  return Response.json({ message: "Processing request", to: to });
}
// 32. Define getGPTResults function
const getGPTResults = async (inputString, to) => {
...
const createRowForGPTResponse = async (to) => {
  ...
  const payload = { type: "GPT", content: "", to: to };
...
const updateRowWithGPTResponse = async (prevRowId, content, to) => {
...
export async function POST(req, res) {
  ...
  sendPayload({ type: "Query", content: message, to: to });
  // 57. Start the search engine to find sources based on the query
  const results = await searchEngineForSources(message, to);
